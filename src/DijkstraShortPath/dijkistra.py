...
def dijkstraLoop(edgesDic, finalNode):
    print "final node", finalNode

    while(next(reversed(exploredPathDistanceDic)) != finalNode):
        length = 0
        dGreedyCriteriaDic = {}
        ...
        if(currentNode != chosenNode):
            ...
            if (not chosenNode in exploredPathDistanceDic.keys() or exploredPathDistanceDic[chosenNode] > dGreedyCriteriaDic[chosenNode]):    
            ...
            exploredPathDistanceDic[chosenNode] = 'inf'
            return 
